...
- **这个 bug 在哪个引擎上？** teable-ee 有两套记录引擎，lab 的 harness 默认走 v1。
  v2 的 bug 必须声明 `routing: "force-v2"` 并在 setup 里 `assertV2Routing()`，否则用例
  会在每一列都绿——问的是从来没有这个 bug 的那套代码。见 `framework/v2-routing.ts`。
