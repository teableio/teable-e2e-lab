...
更新路径上又做了一次，因为那里的"200 但没落全"更隐蔽：`record/update-100-mixed`
的真 seed + 真 verify 中间换一个做了手脚的 execute，三种手法各自报在不同的地方——

| 手法 | 报在哪 |
|---|---|
| 后两批没发出去 | 回声数 50/100、逐格从第 51 行起、**停在旧值的行数 = 50** |
| 全部发出去但漏掉 `Active` 一列 | 只有逐格比对报；回声数 100、停在旧值的行数 = 0 |
| 一行用了不存在的 id | 回声数 99/100、逐格只报第 10 行 |

第二种是这次真正的收获：服务端照样回 200 和 100 条，行数、id 一个不差，只有逐格
比对能看见——而且它报出的是 `expected='<absent>' actual=True`，即"该被清空的格子
没被清空"。只比对期望里有的键就会漏掉这一整类，所以 verify 里做的是双向比对。
...
- **用例还很少**。框架能力已验证，覆盖面才刚开始铺；准数问 `uv run lab list`。
