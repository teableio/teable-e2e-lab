// Full TypeScript type check for the e2e-lab sources, ported from
// teable-perf-lab.
//
// The lab files only resolve inside the teable-ee monorepo (they import
// "@teable/*" packages and reach outside the repo with relative paths like
// "../utils/init-app"). To type-check them standalone, this script copies the
// sources into a temp directory laid out like community/apps/nestjs-backend,
// stubs every external module as `any`, and runs the real type checker.
// External APIs are not validated, but all local types (framework/types.ts
// contracts, case configs, runner results) are — which is where the
// runner<->config discriminated union earns its keep.
import {
  cp,
  mkdir,
  mkdtemp,
...
// Scans every lab source for imports of external packages or of the
...
// Each imported name is declared both as a value and as a type so it works in
// either position; both resolve to `any`.
...
const buildGlobalShims = () =>
  [
    "declare var testConfig: { baseId: string; userId: string; email: string; [key: string]: any };",
    ...VITEST_GLOBALS.map((name) => `declare const ${name}: any;`),
    "",
  ].join("\n");

const stageWorkspace = async (workspaceRoot) => {
  const labRoot = join(workspaceRoot, "test", "e2e-lab");
  ...
  for (const entry of [
    ...
    await cp(join(repoRoot, entry), join(labRoot, entry), {
      recursive: true,
    });
  }

  const sourceFiles = await collectTsFiles(labRoot);
...
const workspaceRoot = await mkdtemp(join(tmpdir(), "e2e-lab-typecheck-"));

try {
  await stageWorkspace(workspaceRoot);

  const rootNames = await collectTsFiles(workspaceRoot);
  // Not strict: the any-typed stubs would trip noImplicitAny/unknown checks
  // everywhere. The goal is catching structural bugs against local types
  // (e.g. a runner returning an object missing BugProbeResult fields), which
  // non-strict checking covers.
  ...
  const diagnostics = ts
    ...
    // fires against the any-typed stubs (e.g. axios.get<T>); with the real
    // monorepo types those calls are fine.
  ...
  const checkedFiles = rootNames.filter((file) =>
    file.includes(join("test", "e2e-lab")),
  );
