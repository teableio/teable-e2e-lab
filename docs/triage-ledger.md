...
| `cfddb0057`  | T6862 | Tried in **three** shapes, all green on the fix's parent `22b1516cd`: deleting a link field then its target (the optional link's FK is `SET NULL`, so the leftover column blocks nothing); the same inside a duplicated base on the theory that the copied key came back `NO ACTION`; and, on 2026-09-11, a duplicate taken **with records** where the deleted row is one the copy's own order actually points at, so the foreign key is genuinely exercised - deleted 200 on both columns, cell cleared (run 34585327257). That third shape was the one the first two left open. Whatever rebuilds the key without `ON DELETE` is not what `POST /base/duplicate` does here. |
| `cfddb0057`  | T6864 | Same two attempts as T6862 above — the two only reproduce together, and neither shape reproduced. T6863 from this same commit **is** covered, by `link/y186-cross-base-link-clears-on-delete`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`lookup/y207-conditional-filter-over-a-foreign-table`. Its runner still supports the
...
`filter/y222-plain-date-string-filters-a-date-column` for the sibling fix T5584;
...
`field/y228-turning-off-no-duplicates-lets-a-duplicate-in`) and once for indexes an
...
the only phase that moved moved in opposite directions at the two fanouts. That
is the run-to-run variation between two local checkouts, not the fix. Its
ledger's reading is that the effect is now smaller than the noise a single
local sample carries, so the next attempt belongs in that lab's same-host
paired base/candidate lane rather than in a larger fixture on either side.
