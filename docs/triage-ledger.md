...
| `56fe8df36` | T4864 | Written and run, red on **both** columns (run 32901128869): asked through the ordinary record endpoint with the link column's view, the product names nothing as linked on develop either. The commit skips the view scope in the **share** read paths, so a case that asks the ordinary endpoint is knocking at a different door; reaching it needs a shared view and its own read endpoints. |
