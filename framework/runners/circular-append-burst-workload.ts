// Pure workload model for the circular-append-burst runner. No Teable I/O.
//
// Ported from teable-perf-lab's circular-link-propagation workload
// (case/circular-storm-repro-hunt), specialized to the one mutation this lab
// needs: appending Purification rows. The fixture is the 2026-08-27 CN
// production incident shape — a four-table graph whose SubOrders and
// Purification tables link to each other in BOTH directions and look each
// other up, with formulas layered on those lookups, plus TWO duplicate
// one-many links from SubOrders to Purification doubling the dependency
// edges. Every field family count matches the incident fingerprint
// (guarded by circular-append-burst-workload.test.js).
//
// Everything here is computable from row numbers and config, so a rerun
// compares byte for byte (Deterministic Data rule).

export type CircularPermutation = { multiplier: number; offset: number };

// "seed" is the fixture before the burst; "appended" is after the burst
// added `appendRowCount` Purification rows extending the same permutation.
export type CircularAppendPhase = "seed" | "appended";

// The scale + mapping subset of the case config the model needs.
export interface CircularAppendWorkloadConfig {
  ...
  // purification row -> subOrder row (must be injective over the seeded AND
  // appended range: multiplier coprime with subOrderRowCount and
  // purificationRowCount + appendRowCount <= subOrderRowCount)
  ...
  // Rows the burst appends (p = purificationRowCount+1 ..), each wiring all
  // four link cells.
...
// Plasmid (16 fields, conditional-lookup source).
...
// The duplicate one-many pair (the incident base carried the same link twice).
...
// Purification (88 fields, 41 computed, the appended table).
...
export const orderRowForSubOrder = (
  subOrderRow: number,
  config: CircularAppendWorkloadConfig,
...
export const purificationRowTotal = (
  config: CircularAppendWorkloadConfig,
  phase: CircularAppendPhase,
) =>
  config.purificationRowCount +
  (phase === "appended" ? config.appendRowCount : 0);

// The appended row numbers, extending the seeded permutation.
export const appendedPurificationRows = (
  config: CircularAppendWorkloadConfig,
) =>
  Array.from(
    { length: config.appendRowCount },
    (_, index) => config.purificationRowCount + index + 1,
  );

// SubOrder rows the burst dirties: the hosts of the appended rows. Each was
// purification-free in the seed phase, so its purification lookups flip from
// empty to real values — a "did propagation arrive at all" observation, not a
// value diff.
export const appendedHostSubOrderRows = (
  config: CircularAppendWorkloadConfig,
) =>
  appendedPurificationRows(config).map((p) =>
    subOrderRowForPurification(p, config),
  );

// Injective purification -> subOrder mapping proof + inverse map, over the
// seeded range plus (in the appended phase) the appended range. Each
// purification attaches to a distinct subOrder, so subOrders carry 0 or 1
// purification and every one-many lookup holds an exact single value.
export const purificationRowBySubOrderRow = (
  config: CircularAppendWorkloadConfig,
  phase: CircularAppendPhase = "seed",
): Map<number, number> => {
...
export const purificationExpressionValue = (p: number) => p * 10;
...
// "formula" marks lookups whose target is a formula field: the server requires
// the lookup's declared `type` to equal the looked-up field's type (a lookup
// of a formula must be created with type "formula"), while its cell value
// stays the formula's string result.
...
// SubOrders: 7 lookups over the one-many purification link, including one
// that pulls the Purification formula `actual_expression` (the
// formula-over-lookup carried through a lookup).
...
// 4 number / 1 link), split across the two symmetric backref links so the
...
// Orders: 6 own-field formulas (schema fidelity; unaffected by the burst).
...
// Purification's lu_so_expression_card); so_is_expressible is the field the
// incident reproduction saw stuck on "NO-..." forever.
...
// Purification: 14 formulas. actual_expression is the formula the SubOrders
// lookup pulls; p_chain_card sits on the reverse lookup of the SubOrders
// formula, closing the circular dependency SubOrders <-> Purification.
...
// ---------------------------------------------------------------------------
// Field-count audit (kept executable so drift from the incident fingerprint
// fails the unit test, not a human review).
// ---------------------------------------------------------------------------
...
export type ExpectedCell =
  ...
  | { kind: "empty" }
  // Deliberately unasserted: rendering of blank-lookup formulas is not part
  // of the case contract (see the case markdown assumptions).
  | { kind: "skip" };
...
export const expectedActualExpression = (
  ...
  `AE${purificationExpressionValue(p)} ${subOrderText(
    1,
    subOrderRowForPurification(p, config),
  )}`;
...
export const expectedSubOrderExpressionCard = (
  ...
  p: number,
  config: CircularAppendWorkloadConfig,
) => `SO ${subOrderTitle(s)} :: ${expectedActualExpression(p, config)}`;

// Expected computed state of one SubOrders row: field name -> expectation.
// `purificationRow` is the row linked to this host in the asserted phase, or
// undefined for a purification-free host.
export const expectedSubOrderComputed = (
  ...
  if (p === undefined) {
    ...
    expected.lu_p_expression_mg_l = value(purificationExpressionValue(p));
    ...
    expected.lu_p_actual_expression = value(
      expectedActualExpression(p, config),
    );
    expected.so_expression_card = value(
      expectedSubOrderExpressionCard(s, p, config),
    );
...
// Expected computed state of one Purification row (seeded or appended — both
// follow the same algebra, which is what makes the appended rows' own 41
// computed fields assertable).
export const expectedPurificationComputed = (
  ...
  for (const spec of PURIFICATION_SUBORDER_LOOKUPS) {
    if (spec.target === TITLE_FIELD) {
      ...
      expected[spec.name] = value(expectedSubOrderExpressionCard(s, p, config));
    } else {
  ...
  expected.actual_expression = value(expectedActualExpression(p, config));
  expected.p_chain_card = value(
    `P ${purificationTitle(p)} :: ${expectedSubOrderExpressionCard(s, p, config)}`,
  );
