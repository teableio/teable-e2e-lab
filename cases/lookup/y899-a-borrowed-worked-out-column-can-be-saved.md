...
Three tables. A leaf table holds a number; a source table borrows that number
and works something out from what it borrowed; the host borrows that worked-out
column. Then the ordinary save from the host column's settings screen.

The three tables are the point. Four shapes of this fixture have been measured:
a one-column formula renamed, re-pointed, and converted away (runs 32675528990,
32675852808, 32676121196), and a two-column formula renamed and reformatted (run 34582424969) — all accepted on the fix's parent. What is left is an instruction
that already names another table's column before it is copied anywhere, which is
what this fixture builds. Then the ordinary save from the column's
