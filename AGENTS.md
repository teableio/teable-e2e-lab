...
- Security-sensitive bugs are not accepted before their fix ships (see
  [CONTRIBUTING.md](CONTRIBUTING.md)).
- No internal URLs, customer data, or credentials in cases or docs. The
  repository's three secrets — `TEABLE_EE_CHECKOUT_TOKEN` (fetching teable-ee
  sources), `TEABLE_E2E_LAB_TOKEN` (writing the results table), and
  `FEISHU_E2E_WEBHOOK_URL` (posting the card) — live only in GitHub secrets.
- Everything committed here is written in English: code, comments, docs, case
  titles, and the strings the reports emit. `pnpm check:english` enforces it.
